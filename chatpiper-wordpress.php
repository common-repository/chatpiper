            ...
            function () {
                ...
                <div class="chatpiper-form">
                    ...
                    <form name="options" method="POST" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
                        ...
                        <input type="text" name="chatpiper_url" placeholder="https://chat.mysite.net" value="<?php echo get_option( 'CHATPIPER_URL', '' ); ?>" size="70">
...
add_filter( 'script_loader_tag', function ( $tag, $handle ) {
    ...
    if ( 'chatpiper-script' !== $handle )
        return $tag;

    return str_replace( ' src', ' id="chatPiperScript" src', $tag );
}, 10, 2 );

add_action( 'wp_enqueue_scripts', function () {

		if( get_option('CHATPIPER_URL') === false)
			return;
		wp_enqueue_script('chatpiper-script',
			get_option('CHATPIPER_URL') . '/chatpiper-script', array(),
			null, true);
